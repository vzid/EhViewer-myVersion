...
画廊详情页可长按预览图选择页面，并通过右下角下载按钮保存阅读页清晰度的图片，不会强制走原图下载。已选页面会保留到全部下载成功为止，也可在阅读器底部选择框中联动调整；通过此方式下载过的本子，下次进入时会提示下载时间。下载中再次点击选中下载按钮会停止当前批次；重新点击下载时，会按图片名跳过已保存图片，并继续下载缺失或新加入选择的页面。
