...
画廊详情页可长按预览图选择页面，并通过右下角下载按钮保存阅读页清晰度的图片，不会强制走原图下载。左下角清空选择图标按钮可一次清除当前本子的选择。已选页面会保留到全部下载成功为止，也可在阅读器底部选择框中联动调整。选中图片下载会在离开本子详情页后继续运行，并在通知栏显示进度、网速和完成结果；重新点击下载时，会按保存文件名跳过已保存图片，并继续下载缺失或新加入选择的页面。图片会保存到 `Pictures/EhViewer/{artist 或 group 或 unknow}-{标题名}/`，文件名按总页数位数补齐为 `001.png` 这类格式。通过此方式下载过的本子，下次进入时会提示下载时间。下载中再次点击选中下载按钮会停止当前批次并保留选择。
