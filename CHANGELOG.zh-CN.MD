...
* 选择画廊预览图时，将高清下载入口改为右下角小悬浮按钮
* 选择画廊预览图时支持切换点选/阅读模式、在阅读器联动勾选，并保留未完成下载的选择
* 从画廊预览图进入阅读器后，返回详情页会定位到最后阅读的页面缩略图
* 选择画廊预览图时新增左下角清空选择图标按钮
* 选择画廊预览图下载时支持再次点击停止，重试时按图片名跳过已保存图片
* 选择画廊预览图下载时离开详情页仍会继续下载，并在通知栏显示进度、网速和完成结果
* 选择画廊预览图下载时按 `{artist/group/unknow}-{标题名}` 建文件夹，并以补齐页码的 `001.png` 形式保存
* 记录通过预览图高清下载完成的本子，并在再次进入时提示下载时间
